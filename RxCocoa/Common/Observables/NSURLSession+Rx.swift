...
func convertResponseToString(data: NSData!, _ response: URLResponse!, _ error: NSError!, _ interval: TimeInterval) -> String {
    ...
    public func rx_response(request: URLRequest) -> Observable<(Data, HTTPURLResponse)> {
        return Observable.create { observer in
            ...
            let task = self.dataTask(with: request) { (data, response, error) in
                ...
                if Logging.URLRequests(request) {
                    let interval = NSDate().timeIntervalSince((d ?? NSDate()) as Date)
